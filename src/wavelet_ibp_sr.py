...
def wavelet_sr_hybrid_gray(img_lr: np.ndarray,
    ...
    Steps:
      - Bilinear upsample
      - DWT on upsampled image (HF bands)
      - SWT on upsampled image (edge guidance)
      - Soft-threshold HF
      - Edge-guided gain on HF
      - IDWT reconstruct
    """
    if scale != 2:
...
def iterative_back_projection_Y(Y_sr: np.ndarray,
    ...
    """
    Iterative Back-Projection on luminance channel.
    Enforces that downsample(blur(SR)) ~= LR.
    """
...
def run_wavelet_ibp_sr(
    ...
    if use_degradation:
        # HR olarak input'u kabul ediyoruz, LR'ı simüle ediyoruz
        hr_rgb = img_rgb
...
# =========================
# Optional: local test/eval
# =========================
def main():
    # imports only for local testing
    import matplotlib.pyplot as plt
    from skimage.metrics import peak_signal_noise_ratio as psnr

    hr_path = "kedi.jpg"  # HR image for simulated evaluation
    scale = 2
    sigma_lr = 1.0
